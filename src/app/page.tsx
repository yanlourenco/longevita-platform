...
// Mock de Assistidos Cadastrados da Família
...
export default function LongeVitaDashboard() {
  ...
  // Carrega se houver assistido cadastrado na sessão
  useEffect(() => {
    try {
  ...
  const handleHire = (type: string) => {
    setHiredStatus(type);
    success("Contrato LGPD Gerado!", `Vínculo com ${activeCuidador?.nome} registrado no Supabase.`);
    setTimeout(() => {
      setHiredStatus(null);
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfd] selection:bg-[#72b63f] selection:text-white pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12">
        {/* ABA 1: MEUS ASSISTIDOS (PAINEL DO CONTRATANTE) */}
        {activeTab === "assistidos" && (
          <motion.div
        ...
        {/* ABA 2: CUIDADORES DISPONÍVEIS (APPLE SPRING RIG) */}
        {activeTab === "cuidadores" && (
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {/* ABA 3: DIÁRIO DE BORDO EM TEMPO REAL */}
        {activeTab === "diario" && (
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
